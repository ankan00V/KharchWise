  ...
  return (
    <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-max min-w-[320px] max-w-[800px]">
      <motion.nav 
        ...
        className="bg-[rgba(7,7,9,0.5)] backdrop-blur-3xl border border-[rgba(255,255,255,0.08)] rounded-full pl-5 pr-3 py-2 shadow-[0_16px_40px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.15),inset_0_0_0_1px_rgba(255,255,255,0.02)] flex items-center justify-between gap-12"
      >
        <Link to="/groups" className="flex items-center no-underline hover:opacity-80 transition-opacity">
          <KharchwiseLogo size="sm" showLabel={true} />
        </Link>
        <div className="flex items-center gap-4">
          {user ? (
            <>
              <span className="text-[rgba(255,255,255,0.8)] font-sans font-semibold text-[14px]">{user.name}</span>
              <Button
